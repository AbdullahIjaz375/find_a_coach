<template>
  <form @submit.prevent="submitForm">
    <div class="form-control" :class="{ invalid: !firstName.isValid }">
      ...
      <p v-if="!firstName.isValid">First Name must be added.</p>
    </div>
    <div class="form-control" :class="{ invalid: !lastName.isValid }">
      <label for="lastname">Lastname</label>
      <input type="text" id="lastname" v-model="lastName.val" />
      <p v-if="!lastName.isValid">Last Name must be added.</p>
    </div>
    <div class="form-control" :class="{ invalid: !description.isValid }">
      <label for="description">Description</label>
      <textarea id="description" rows="5" v-model="description.val"></textarea>
      <p v-if="!description.isValid">Description must be added.</p>
    </div>
    ...
    <div class="form-control" :class="{ invalid: !areas.isValid }">
      ...
      <div>
        <input type="checkbox" id="mobile" value="mobile" v-model="areas.val" />
        <label for="mobile"> Mobile Development</label>
      </div>
      <p v-if="!areas.isValid">
        Atleast one of the given areas of expertise need to be selected.
      </p>
    </div>
